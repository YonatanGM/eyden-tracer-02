...
/*
			L_i = light->Illuminate(I).value();
			R_v = -I.dir + 2 * I.dir.dot(m_Normal) * m_Normal;
			
			i_N = I.dir.dot(m_Normal);
			if(i_N > 0)
				diffuse += L_i * i_N;
			
			specular += L_i * pow(-ray.dir.dot(normalize(R_v)), m_ke);
*/
		}
